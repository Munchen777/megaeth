...
func MintMegaCatNFT(accountData accTypes.AccountData) (bool, error) {
	log.Infof("[%d/%d] | %s | [MintMegaCatNFT] | Start Minting Mega Cat NFT ...\n",
	...
	if err != nil {
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Problem with encoding parameters\n",
			global.CurrentProgress, global.TargetProgress, accountData.AccountAddress,
		)
		log.Error(msg)
	...
	if !ok {
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Problem with client initialization\n",
			global.CurrentProgress, global.TargetProgress, accountData.AccountAddress,
		)
		log.Error(msg)
		return false, errors.New(msg)
	...
	tx, err := client.BuildTransaction(
		"0x0837ec39d40CCdcea4b4B6bfCfb3d71E7EbFC71C",
		data,
		big.NewInt(0),
	)

	if err != nil {
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Problem with building transaction: %v\n",
	...
	if err != nil {
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Problem with getting chainID: %v\n",
			global.CurrentProgress, global.TargetProgress, accountData.AccountAddress, err,
		)
		log.Error(msg)
		return false, errors.New(msg)
	...
	if err != nil {
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Problem with signing tx: %v\n",
			global.CurrentProgress, global.TargetProgress, accountData.AccountAddress, err,
		)
		log.Error(msg)
		return false, errors.New(msg)
	...
	if err != nil {
		...
		msg := fmt.Sprintf("[%d/%d] | %s | [MintMegaCatNFT] | Successfully executed transaction\n",
			global.CurrentProgress, global.TargetProgress, accountData.AccountAddress,
		)
		log.Info(msg)
	}
